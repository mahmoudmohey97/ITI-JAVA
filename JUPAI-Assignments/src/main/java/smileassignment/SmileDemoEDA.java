...
public class SmileDemoEDA {
    String trainPath = "src/main/resources/titanic_train.csv";
    String testPath = "src/main/resources/titanic_test.csv";

    public static void main(String[] args) throws IOException {
        SmileDemoEDA sd = new SmileDemoEDA ();
        //SmileDemoEDA sdTest = new SmileDemoEDA();
        PassengerProvider pProvider = new PassengerProvider ();
        DataFrame trainData = pProvider.readCSV (sd.trainPath);
        trainData = trainData.select ("Name", "Age","Pclass", "Sex", "Survived");
        PassengerProvider pProvider2 = new PassengerProvider ();
        DataFrame testData = pProvider2.readCSV(sd.testPath);
        testData = testData.select ("Name", "Pclass", "Sex", "Survived");
        ...
        int [] target = testData.column("Survived").toIntArray();
        testData = testData.drop("Survived");
        ...
        System.out.println(Accuracy.of(target, pred));
    }

    public static int[] encodeCategory(DataFrame df, String columnName) {
        System.out.println(df.stringVector (columnName).distinct ().toString());
